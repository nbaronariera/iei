    ...
    public class GALParser : Parser<GALData>
    {
        ...
        public List<ResultObject> FromParsedToUsefull(List<GALData> datosParseados)
        {
            ...
            foreach (var dato in datosParseados)
            {
                ...
                if (string.IsNullOrWhiteSpace(dato.Provincia)) resultadoDebug.Motivos.Add("Provincia vacía o nula.");
                if (string.IsNullOrWhiteSpace(dato.Municipio)) resultadoDebug.Motivos.Add("Municipio vacío o nulo.");
                ...
                {
                    var nuevasCoords = seleniumHelper.ObtenerCoordenadas(dato.Direccion, dato.Municipio);
                    if (EsCoordenadaEnEspañaPeninsular(nuevasCoords.Lat, nuevasCoords.Lng))
                    {
                        lat = nuevasCoords.Lat;
                        lon = nuevasCoords.Lng;
                    }
                }
        ...
        private Provincia ObtenerOCrearProvincia(AppDbContext ctx, string nombre)
        {
            ...
            if (string.IsNullOrWhiteSpace(nombre)) nombre = "Provincia desconocida";
            nombre = nombre.Trim();
        ...
        private double ParsearCoordenadaDMS(string dms)
        {
            ...
            dms = dms.Replace("º", "°").Replace("", "°").Replace("’", "'").Replace("′", "'").Replace("“", "\"").Replace("”", "\"").Replace(",", ".").Trim();
            if (double.TryParse(dms, NumberStyles.Any, CultureInfo.InvariantCulture, out double dec)) return dec;
            var match = Regex.Match(dms, @"(-?\d+(?:\.\d*)?)\s*°\s*(\d+(?:\.\d*)?)?");
            if (match.Success)
            {
                double grados = double.Parse(match.Groups[1].Value, CultureInfo.InvariantCulture);
                double minutos = match.Groups[2].Success ? double.Parse(match.Groups[2].Value, CultureInfo.InvariantCulture) : 0;
        ...
        private string ConvertirHorario(string raw)
        {
            ...
            raw = raw.ToLower().Replace(" horas", "").Replace(" h.", "").Replace(".", ":");
            var diasMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { { "de luns a venres", "L-V" }, { "sábados", "S" } };
            var bloques = Regex.Matches(raw, @"(?<rangos>.*?)\((?<dias>.*?)\)").Cast<Match>().Select(m => new { RangosRaw = m.Groups["rangos"].Value, DiasRaw = m.Groups["dias"].Value.Trim() }).ToList();
            if (!bloques.Any()) return "";
            var resultadoFinal = new List<string>();
            foreach (var bloque in bloques)
            {
                string etiquetaDias = diasMap.Where(kv => bloque.DiasRaw.Contains(kv.Key, StringComparison.OrdinalIgnoreCase)).Select(kv => kv.Value).FirstOrDefault() ?? bloque.DiasRaw;
                var rangos = Regex.Matches(bloque.RangosRaw, @"(\d{1,2}[:]\d{1,2})\s*a\s*(\d{1,2}[:]\d{1,2})", RegexOptions.IgnoreCase).Cast<Match>().Select(m => $"{m.Groups[1].Value}-{m.Groups[2].Value}").ToList();
                if (rangos.Count > 0) resultadoFinal.Add($"{etiquetaDias} ({string.Join(",", rangos)})");
            }
        ...
        private string NormalizarHora(string h)
        {
            ...
            if (!h.Contains(":")) h += ":00";
            var parts = h.Split(':');
            if (parts.Length == 2 && parts[1].Length == 1) return $"{parts[0]}:0{parts[1]}";
            return h;
        }
        ...
        private bool EstacionYaExiste(AppDbContext ctx, string nombre, double lat, double lon)
        {
            ...
            return ctx.Estaciones.Any(e => e.nombre.Trim().ToLower() == nombreNorm || (Math.Round(e.latitud, 6) == latNorm && Math.Round(e.longitud, 6) == lonNorm));
        }
        ...
        private void MostrarResumen(List<ResultadoDebug> resultados)
        {
            ...
            Debug.WriteLine("\n ESTACIONES AÑADIDAS\n------------------------------------------------------------");
            foreach (var r in añadidas) Debug.WriteLine($"{r.Nombre,-35} | {r.Provincia,-12} | {r.CodigoPostal,-6} | {string.Join("; ", r.Motivos)}");
            Debug.WriteLine("\n ESTACIONES DESCARTADAS\n------------------------------------------------------------");
            foreach (var r in descartadas) Debug.WriteLine($"{r.Nombre,-35} | {r.Provincia,-12} | {r.CodigoPostal,-6} | {string.Join("; ", r.Motivos)}");
